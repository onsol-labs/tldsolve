...
describe('tldSolve tests', () => {
    ...
    it('should perform retrieval of all sol user domains', async () => {
        const solver = new TldSolve(connection);
        const allDomains = await solver.getAllDomainsFromUser(
            owner,
            Protocol.SNS,
        );
    ...
    it('should perform retrieval of ans main domain', async () => {
        ...
        expect(mainDomain?.domain).toStrictEqual(
            expect.stringContaining('miester'),
        );
    });
    ...
    it('should perform retrieval of NameRecordHeader of miester.abc', async () => {
        ...
        expect(resolvedDomain?.owner?.toBase58()).toStrictEqual(
    ...
    it('should perform retrieval of nameAccount of miester.abc', async () => {
        const solver = new TldSolve(connection);
        const nameAccount = await solver.getNameAccountFromDomain(
            'miester.abc',
        );
        // console.log("nameAccount: ", nameAccount?.toBase58())
        const miesterDotAbc = new PublicKey(
            '9YzfCEHb62bQ47snUyjkxhC9Eb6y7CSodK3m8CKWstjV',
        );
        expect(nameAccount?.toBase58()).toStrictEqual(
            expect.stringContaining(miesterDotAbc.toBase58()),
        );
    });
    it('should perform retrieval of nameAccount of miester.sol', async () => {
        ...
        // console.log("nameAccount: ", nameAccount?.toBase58())
        const miesterDotSol = new PublicKey(
            'ErBfMkE1YJiqG1wxBr6TAG15WrTtcrWS3pZbEKKFRmUr',
        );
        expect(nameAccount?.toBase58()).toStrictEqual(
            expect.stringContaining(miesterDotSol.toBase58()),
        );
    });
    it('should perform retrieval of allAbcDomains owner in abc', async () => {
        const solver = new TldSolve(connection);
        const allAbcDomains = await solver.getAllDomainsFromUserFromTld(
            owner,
            'abc',
        );
    ...
    it('should perform retrieval of allSNSDomains owner in sol', async () => {
        const solver = new TldSolve(connection);
        const allSNSDomains = await solver.getAllDomainsFromUserFromTld(
            owner,
            'sol',
        );
    ...
    it('should perform retrieval of domain name from SNS nameAccount', async () => {
        ...
        const domain = await solver.reverseLookupNameAccount(
            miesterDotSol,
            Protocol.SNS,
        );
        // console.log("domain: ", domain)
