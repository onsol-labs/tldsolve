import { getDomainKey as getSPLDomainKey, NameRegistryState, getAllDomains as getAllSPLDomains, performReverseLookup as performSPLReverseLookup } from '@bonfida/spl-name-service';
import { findOwnedNameAccountsForUser, getDomainKey, NameRecordHeader, TldParser } from '@onsol/tldparser';
...
export class TldSolve {
  ...
  async resolveDomain(domain: string): Promise<NameRecordHeader | NameRegistryState | undefined> {
  ...
  async getAllDomainsFromUserFromTld(
    ...
    const domainsANS = await parser.getAllUserDomainsFromTld(userAccount, tld)
    return domainsANS;
  }
  ...
  async reverseLookupNameAccountWithKnownParent(
    ...
    const parser = new TldParser(this.connection);
    const domainName = await parser.reverseLookupNameAccount(nameAccount, parentAccountOwner)
    if (!domainName) {
      if (typeof nameAccount == 'string') {
        nameAccount = new PublicKey(nameAccount);
      }
      const domainName = await performSPLReverseLookup(this.connection, nameAccount);
      return domainName
    }
    return domainName;
  }
  ...
  async reverseLookupNameAccount(
    ...
    const parser = new TldParser(this.connection);
    const tld = await parser.getTldFromParentAccount(nameRecordHeader.parentName);
    const [tldHouse] = findTldHouse('.' + tld);
    const domainName = await parser.reverseLookupNameAccount(nameAccount, tldHouse)
    if (!domainName) {
      if (typeof nameAccount == 'string') {
        nameAccount = new PublicKey(nameAccount);
      }
